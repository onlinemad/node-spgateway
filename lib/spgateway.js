...
spgateway.checkvalue = (key, iv, payload) => {
  return crypto.createHash('sha256').update(qs.stringify(Object.assign({ HashKey: key }, Object.keys(payload).sort().reduce((r, k) => (r[k] = payload[k], r), {}), { HashIV: iv }))).digest('hex').toUpperCase()
}

spgateway.checkcode = (key, iv, payload) => {
  return crypto.createHash('sha256').update(qs.stringify(Object.assign({ HashIV: iv }, Object.keys(payload).sort().reduce((r, k) => (r[k] = payload[k], r), {}), { HashKey: key }))).digest('hex').toUpperCase()
}
